...
let tasks: Task[] = Array.from({ length: 12 }, (_, i) => ({
  ...
  title: `Task ${i + 1}`,
  description: `This is the description for task number ${i + 1}. It contains some details about what needs to be done.`,
  createdAt: new Date(Date.now() - (12-i) * 24 * 60 * 60 * 1000),
  updatedAt: new Date(Date.now() - (12-i) * 24 * 60 * 60 * 1000),
}));

let auditLogs: AuditLog[] = [];

// Simulate some initial audit logs
